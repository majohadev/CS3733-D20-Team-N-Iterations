...
public class ServiceController {
  ...
  public static boolean addService(int id, String name, int years, String language, char gender, Timestamp start, Timestamp end, String type) throws DBException {
    try{
  ...
  public static boolean modifyService(int id, String name, int years, String lang, char gender, Timestamp start, Timestamp end, String type) throws DBException {
    try{
      String query = "UPDATE employees SET employeeID = ?, name = ?, yearsofExperience =?, lang = ?, gender = ?, start = ?, end = ?, type = ?";
      PreparedStatement state = con.prepareStatement(query);
      state.setInt(1, id);
      state.setString(2, name);
      state.setInt(3, years);
  ...
  public static boolean modifyLanguage(int id, String lang) throws DBException {
    try{
      return modifyService(id, getService(id).getName(), getService(id).getYearsofExperience(), lang, getService(id).getGender(), getService(id).getStart(), getService(id).getEnd(), getService(id).getType());
    } catch (DBException e) {
      e.printStackTrace();
  ...
  public static List<Employee> searchbyLang(String lang) throws DBException {
    ...
    if(queries.size() == 0)
      throw new DBException("Error: size is wrong in searchbyLang method");
