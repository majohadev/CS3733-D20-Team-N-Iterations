...
public abstract class AbsAlgo implements IPathFinder {
  ...
  /**
   * Gets all of the elevator and stair nodes accessible to the given node. If the given node isn't an elevator or stair, returns null.
  ...
  public static LinkedList<DbNode> searchAccessible(DbNode node){
    ...
    while(queue.size() > 0){
      ...
      try {
        next = MapDB.getGAdjacent(cur.getNodeID(), -1, -1); //only gets elevator and stair nodes; kinda a hack, but not actually that bad
      }catch(DBException e){
